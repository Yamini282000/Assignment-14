...
\section{Solution}
\begin{lemma}
The conditional probability $Pr(A|B)$ equals $Pr(A)$ for independent events. \label{L1}
\end{lemma} 
Let the random variable representing the events be $X \in \{0,1,2\}$,where
\begin{table}[ht!]
\begin{tabular}{|l|l|l|}
\hline
\multirow{3}{*}{X} & 0 & Number appearing is a multiple of 3        \\ \cline{2-3} 
                   & 1 & Number appearing is even                   \\ \cline{2-3} 
                   & 2 & Number appearing is even and multiple of 3 \\ \hline
\end{tabular}
\end{table}
\\
...
\begin{align}
   Pr(X=0|X=1) = \frac{Pr(X=2)}{Pr(X=1)}= \frac{1}{3}
   \\ = Pr(X=0)
\end{align}
